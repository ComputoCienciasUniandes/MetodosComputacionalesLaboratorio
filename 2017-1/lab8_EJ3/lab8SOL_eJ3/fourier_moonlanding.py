...
# Show only where 95% of the power is, because there are a few huge spikes that otherwise make it impossible to see anything
power_cut = 95.0
clipped_power = mlab.prctile(power.flatten(), power_cut)
img.set_clim(0, clipped_power)

ax[1,1].set_title('Filtered Spectrum')
img = ax[1,1].imshow(abs(im_f_clean)**2, cmap=cm.Blues)
img.set_clim(0, clipped_power)
